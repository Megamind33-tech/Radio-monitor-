...
PLAYLIST_URL = "https://onlineradiobox.com/zm/mano893/?cs=zm.qzambia&played=1"
SOURCE_PROVIDER = "onlineradiobox_mano893_weekly"
...
def get_weekly_tracks():
    ...
    raw = mod.fetch_html(mod.SOURCE_URL)
    recent = mod.parse_orb_tracks(raw)
    ...
    pages = [{"url": mod.SOURCE_URL, "label": "main_played_working_parser", "count": len(out)}]
    return pages, out
...
def update_current(conn, tracks):
    ...
    latest = sorted(tracks, key=lambda x: x["observedAt"], reverse=True)[0]
...
def main():
    ...
    try:
        ...
        if args.sync_current:
            current = update_current(conn, tracks)
